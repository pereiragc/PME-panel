...
#+begin_src R :session :tangle lib_oldpme.r :results silent
oldpme_getweights <- function(pp_old, newcname = "PESOm") {
  ...
  if (!(length(anomalous_years) == 0)) {
    message(glue::glue("Found anomalies in years {anomalous_years}"))
  }
...
dtweights <- pp_old[, .(weight = PESOm[1]), .(.year, .state, V105)]
...
dtnppl <- hh_old[, .(estimate_pop = unique(V600)[1],
                     ...
                     given_weight = PESOm[1]), .(.year, .state, V105)]
#+end_src
