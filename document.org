...
First, set the household key in the household dataset:
#+begin_src R :session :results silent
...
Great! Now we combine households and persons in the 1991-2k range.

#+begin_src R :session :results silent

pp_old[is.na(n_entry), `:=`(
                         V105m = MÊS,
                         V102m = V102,
                         V103m = V103
                       )]
...
Note that there is a problem with entries in ~.year == 2000~ generating weird month values, as documented in [[sec:weird_months]]. So
...
2. In years 91-2k, there are minor issues, probably not worth losing sleep over 845 observations.
