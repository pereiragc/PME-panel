...
pp_old[V301 %between% c(4,7) & .PIA == TRUE, labor_force := (V313 == 1)]

pp_old[V301 %between% c(4,7)  & V313 == 1 & .PIA == TRUE, occup := FALSE]
...
#+begin_src R :session :colnames yes
dt_unemp <- melt(pp_old[, .(`Constructed (weighted)` = mean(.weight * !occup, na.rm=TRUE) / mean(.weight * (occup + !occup), na.rm=TRUE),
                            `Constructed (unweighted)` = mean(!occup, na.rm = TRUE)), .(.year, V105)],
                 id.vars = c(".year", "V105"),
...
|--------------------------+-------+------+------------------|
| Constructed (weighted)   |  1984 |    1 | 8.32840174155641 |
| Constructed (weighted)   |  1984 |    2 | 8.97622266724949 |
| Constructed (unweighted) |  1984 |    1 | 8.34040195959631 |
| Constructed (unweighted) |  1984 |    2 | 9.06404434352878 |
...
#+begin_src R :session :colnames yes :results file graphics :file output/unemployment_rate.png
...
ggplot(dt_unemp_plot, aes(x = Date, y = unemp_rate, linetype = Series)) + geom_line()

#+end_src

#+RESULTS:
[[file:output/unemployment_rate.png]]
