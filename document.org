...
|--------------------------+-------+------+------------------|
| Constructed (weighted)   |  1984 |    1 | 8.29792490207276 |
| Constructed (weighted)   |  1984 |    2 | 8.98037659835562 |
| Constructed (unweighted) |  1984 |    1 | 8.31780024472205 |
| Constructed (unweighted) |  1984 |    2 | 9.07450307372515 |
...
** Identifying individuals over time    
First set the persons dataset key to household, year, month:
#+begin_src R :session :results silent 
setkey(pp_old, hhid, .year, V105)
1
#+end_src

The first criterion is (household, date of birth, sex):

#+begin_src R :session :results silent
pp_old[!is.na(hhid) & !is.na(.bdate), crit1 := .GRP, .(hhid, .bdate)]
1
#+end_src
...
pp_old[!is.na(hhid) & !is.na(.bdate), appearance_count1 := .N, .(hhid, .year, V105, crit1)]

pp_old[, .N, appearance_count1]
...
- Each (hhid, .year, V105, crit1) should not appear more than once.
- There are around 640 thousand entries for which either a missing household id or birth date prevents us from idenfitying them.
- For something in the order of 120 thousand entries, the key (sex, date of birth) did not identify individuals within households. 
