...
#+NAME: loadfun
#+begin_src R :session :tangle lib_oldpme.r :results silent output
...
#+begin_src R :session :results silent output :tangle lib_oldpme.r
...
** Apply identification algorithm to full sample
I'm following the procedure outline in the above section.

Add IDs and inspect ambiguity:
#+begin_src R :session :colnames yes
## Remove person_id1 potentially added before
pp_old[, c("person_id1", "ambiguous_id1", "person_id1_m") := NULL]

pp_old[!is.na(V202) & !is.na(hhid) & !is.na(.bdate), person_id1 := .GRP, .(hhid, V202, .bdate)]
pp_old[!is.na(person_id1), ambiguous_id1 := .N > 1, .(person_id1, .year, V105)]
pp_old[!(ambiguous_id1), person_id1_m := person_id1]
pp_old[, .N, ambiguous_id1]
#+end_src
...
That's a solid performance, but let's try to add the second one based on age.

#+begin_src R :session :results silent output
setkey(pp_old, hhid, V202, .age, .year)
pp_old[, .tag_age_change := frollapply(.age, 2,
                                     function(x) {
                                       agediff <- x[2] - x[1]
                                       return(!(agediff %in% c(0,1)))
                                     },
...
## select age changing points
tmpDT <- pp_old[.tag_age_change  > 0][, .(person_id2 = .I, rown)]

## use rolling merge
pp_old <- tmpDT[pp_old[, -".tag_age_change", with = FALSE], on = .(rown), roll = TRUE]
...
#+begin_src R :session :colnames yes
pp_old[!is.na(person_id2), ambiguous_id2 := .N > 1, .(person_id2, .year, V105)] 
pp_old[!(ambiguous_id2), person_id2_m := person_id2]
pp_old[, .N, ambiguous_id2]
#+end_src
...
Now combine the two IDs.


#+begin_src R :session :results silent output
union_ids(pp_old, "person_id1_m", "person_id2_m", "person_id_join")
1
#+end_src

#+RESULTS:
: 1
** Constructing the transitions in labor status 
Let's tag the people each month who have non-missing records that and the prior month

#+begin_src R :session :results silent output
setkey(pp_old, person_id_join, .year, V105)
pp_old[, .labor_status_L1 := data.table::shift(.labor_status, 1), person_id_join]
pp_old[, .fl_available_transition := !is.na(.labor_status) & !is.na(.labor_status_L1)]

1
#+end_src


We encode transition types, assigning labels too:

#+begin_src R :session :colnames yes

pp_old[(.fl_available_transition), transition_code := .GRP, .(.labor_status, .labor_status_L1)]

dict_transition <- pp_old[(.fl_available_transition), .(status_previous = first(.labor_status_L1), status_now = first(.labor_status)), transition_code]    
...
dict_transition[, Label := paste(transition_labels[status_previous], transition_labels[status_now],
                                 sep = " -> ")]
...
#+RESULTS:
| transition_code | status_previous   | status_now        | Label        |
|-----------------+-------------------+-------------------+--------------|
|               1 | self_employed     | OLF               | s.e. -> olf  |
|               2 | OLF               | OLF               | olf -> olf   |
|               3 | OLF               | informal_salaried | olf -> I     |
|               4 | informal_salaried | unemployed        | I -> U       |
|               5 | unemployed        | OLF               | U -> olf     |
|               6 | OLF               | formal_salaried   | olf -> F     |
|               7 | formal_salaried   | formal_salaried   | F -> F       |
|               8 | formal_salaried   | self_employed     | F -> s.e.    |
|               9 | formal_salaried   | OLF               | F -> olf     |
|              10 | OLF               | unemployed        | olf -> U     |
|              11 | unemployed        | informal_salaried | U -> I       |
|              12 | informal_salaried | informal_salaried | I -> I       |
|              13 | informal_salaried | formal_salaried   | I -> F       |
|              14 | self_employed     | formal_salaried   | s.e. -> F    |
|              15 | formal_salaried   | informal_salaried | F -> I       |
|              16 | informal_salaried | self_employed     | I -> s.e.    |
|              17 | self_employed     | self_employed     | s.e. -> s.e. |
|              18 | informal_salaried | OLF               | I -> olf     |
|              19 | unemployed        | formal_salaried   | U -> F       |
|              20 | self_employed     | unemployed        | s.e. -> U    |
|              21 | OLF               | self_employed     | olf -> s.e.  |
|              22 | formal_salaried   | unemployed        | F -> U       |
|              23 | unemployed        | self_employed     | U -> s.e.    |
|              24 | self_employed     | informal_salaried | s.e. -> I    |
|              25 | unemployed        | unemployed        | U -> U       |


Build a data.table with the relevant months/years and transition codes:

#+begin_src R :session :results silent output

DT <- as.data.table(
  expand.grid(
    transition_code = seq(1,25),
    .state = setdiff(pp_old[, unique(.state)], "BR"),
    V105 = seq(1,12),
    .year = pp_old[, seq(min(.year), max(.year))]
  )
) 

DT[, Date := zoo::as.yearmon(paste(.year, V105, sep = "-"))]

## Recover weights
DT[hh_old, .weight := .weight, on = .(.state, .year, V105)]

DT_transitions <- pp_old[, .(count = .N) , .(transition_code, .state, .year, V105)][DT, on = .(transition_code, .state, .year, V105)]

1
    
#+end_src

Aggregate over states using weights:

#+begin_src R :session :exports code :results silent output

DT_transitions_agg <- DT_transitions[, .(count = sum(.weight * count, na.rm = TRUE)) , .(transition_code, .year, V105, Date)]
...
Now let's plot some things. I begin by computing the inflow/outflow data in absolute terms.

#+begin_src R :session :exports code :results silent output

relevant_stati <- c("formal_salaried", "informal_salaried", "self_employed", "unemployed", "OLF")

inflows <- lapply(relevant_stati,
                  function(st) {
                    dict_transition[status_previous == st & status_now != st][
                      DT_transitions_agg, on = .(transition_code), nomatch = NULL
                    ]
                  })

outflows <- lapply(relevant_stati,
                  function(st) {
                    dict_transition[status_previous != st & status_now == st][
                      DT_transitions_agg, on = .(transition_code), nomatch = NULL
                    ]
...
names(inflows) <- names(outflows) <- relevant_stati
...
1. OLF:
   ...
   - inflows
     #+begin_src R :session :results file graphics output :file output/OLF_inflows_total.pdf

ggplot(inflows[["OLF"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")

     #+end_src

     #+RESULTS:
     [[file:output/OLF_inflows_total.pdf]]

   - outflows
     #+begin_src R :session :results file graphics output :file output/OLF_outflows_total.pdf

ggplot(outflows[["OLF"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")
...
2. Unemployment:
   ...
   - inflows
     #+begin_src R :session :results file graphics output :file output/unemployed_inflows_total.pdf

ggplot(inflows[["unemployed"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")

     #+end_src

     #+RESULTS:
     [[file:output/unemployed_inflows_total.pdf]]

   - outflows
     #+begin_src R :session :results file graphics output :file output/unemployed_outflows_total.pdf

ggplot(outflows[["unemployed"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")
...
3. Formal salaried
   ...
   - inflows
     #+begin_src R :session :results file graphics output :file output/formal_salaried_inflows_total.pdf

ggplot(inflows[["formal_salaried"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")

     #+end_src

     #+RESULTS:
     [[file:output/formal_salaried_inflows_total.pdf]]

   - outflows
     #+begin_src R :session :results file graphics output :file output/formal_salaried_outflows_total.pdf

ggplot(outflows[["formal_salaried"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")
...
4. Informal salaried:
   ...
   - inflows
     #+begin_src R :session :results file graphics output :file output/informal_salaried_inflows_total.pdf

ggplot(inflows[["informal_salaried"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")

     #+end_src

     #+RESULTS:
     [[file:output/informal_salaried_inflows_total.pdf]]

   - outflows
     #+begin_src R :session :results file graphics output :file output/informal_salaried_outflows_total.pdf

ggplot(outflows[["informal_salaried"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")
...
5. Self-employed:
   ...
   - inflows
     #+begin_src R :session :results file graphics output :file output/self_employed_inflows_total.pdf

ggplot(inflows[["self_employed"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")

     #+end_src

     #+RESULTS:
     [[file:output/self_employed_inflows_total.pdf]]

   - outflows
     #+begin_src R :session :results file graphics output :file output/self_employed_outflows_total.pdf

ggplot(outflows[["self_employed"]], aes(x = Date, y = count)) + geom_area(aes(fill = Label)) +
  theme_bw() + scale_fill_brewer(palette = "Set2")
